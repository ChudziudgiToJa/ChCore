...
public class OverWorldDropGui {
    ...
    private static final ItemStack world = new ItemBuilder(Material.GRASS_BLOCK)
            .setTitle("&6świat &8(&7Ziemia&8)")
            .addLore(
        ...
        for (Drop drop : dropConfig.getOverWorldDropList()) {
            inv.setItem(getSlotList()[i++], new ItemBuilder(
                    ...
                    .addLore("&7Szansa: &f" + drop.getChance() + "%",
                            ...
                            "",
                            "&7Status: " + (user.isDropEnabled(drop) ? "&awlaczony" : "&cwylaczony"))
                    .setGlow(user.isDropEnabled(drop))
                    .build(), e -> {
