...
public class NetherDropGui {
    ...
    private static final ItemStack world = new ItemBuilder(Material.NETHERRACK)
            .setTitle("&6świat &8(&7Piekło&8)")
            .addLore(
                    "&7Kliknij aby zmienić drop na..",
                    "&cPiekło &8➝ &aZiemia"
            )
            .build();
    ...
    public static void open(final Player player) {
        ...
        for (Drop drop : dropConfig.getNetherDropList()) {
            inv.setItem(getSlotList()[i++], new ItemBuilder(
                    drop.getItemStack().getType(),
                    drop.getItemStack().getDurability()).setTitle("&f" + drop.getName())
                    .addLore("&7Szansa: &f" + drop.getChance() + "%",
                            "&7Ilosc: &f" + drop.getMinAmount() + "-" + drop.getMaxAmount(),
                            "&7exp: &7" + drop.getExp(),
                            "&7Fortuna: " + (drop.isFortune() ? "&aTAK" : "&cNIE"),
                            "",
                            "&fFortuna zwieksza drop&8:",
                            " &eFortuna I &8(&f+0.5%&8)",
                            " &eFortuna II &8(&f+0.10%&8)",
                            " &eFortuna III &8(&f+0.15%&8)",
                            "",
                            "&7Status: " + (user.isNetherDropEnabled(drop) ? "&awlaczony" : "&cwylaczony"))
                    .setGlow(user.isNetherDropEnabled(drop))
                    .build(), e -> {
