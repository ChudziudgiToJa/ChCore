...
public class BackupGui {
    ...
    public static void openBackup(final Player player, final Player target) {
        ...
        final InventoryBuilder inv = new InventoryBuilder("&9BackUpy: &f" + target.getName(), 9 * 3);
        ...
        for (Backup backup : user.backupList) {
            ItemStack itemStack = new ItemBuilder(Material.SKELETON_SKULL)
                    .setTitle(backup.getInstant().toString())
                    .addLore("",
    ...
    public static void giveBackup(final Player player, final Player target, final Backup backup) {
        ...
        final InventoryBuilder inv = new InventoryBuilder("&9BackUpy: &f" + target.getName() + "&7Z dnia: &f" + backup.getInstant().toString(), 9 * 6);
        int i = 0;
        ...
        inv.setItem(49, new ItemBuilder(Material.EXPERIENCE_BOTTLE).setTitle("&7lvl: &3" + backup.getLvl() ).build(), event -> {
        });
        ...
        inv.setItem(53, new ItemBuilder(Material.LIGHT_BLUE_DYE).setTitle("&3Nadaj ekwipunek").addLore("", "&7Kliknij &3▜&7▛, aby nadać ekwipunek", "").build(), event -> {
                    player.closeInventory();
                    ChatUtil.success(player, "Nadano zapasowy ekwipunek z dnia: " + backup.getInstant().toString() + " dla " + target.getName());
                    backupManager.giveBackup(target, backup);
                }
        );
        inv.open(player);
    }
